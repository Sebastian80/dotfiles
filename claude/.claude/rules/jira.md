...
- **QA handover is a form.** Before any `Send to QA` on an eCom project, load `netresearch-jira`
  (not only `jira-communication`), complete the form in its `references/ecom/qa-workflow.md`, and
  require `QA-GATE-RESULT: pass` from `jira-qa-gate.py KEY`. (A ticket once reached Waiting for QA
  with an empty UAT field and no MR link; the gate found both when run afterwards.)
