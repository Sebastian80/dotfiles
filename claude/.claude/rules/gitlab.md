...
- In skill repos following the skill-repo standard, local `validate-skill.sh` green ≠ CI green: CI fetches the validator from skill-repo-skill `@main`, while the plugin-cache copy lags (a manifest-sync check that existed only in `@main` failed two pipelines after triple local green). Validate with a fresh skill-repo-skill checkout or treat the pipeline as the only authority — and these repos carry **two** plugin manifests (root `plugin.json` + `.claude-plugin/plugin.json`) whose shared fields must stay in sync (`sync-plugin-manifest.sh`).
