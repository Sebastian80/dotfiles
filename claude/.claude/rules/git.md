...
- Iterating on a look, a layout or config values? Stay local until the result is one you want, then squash the session into a single commit. `git revert` is for backing out something already published that others may have pulled, never for undoing your own experiment from ten minutes ago — and never reach for a history rewrite to tidy churn you could have avoided pushing. (Four days of herdr theming left 14 of 340 dotfiles commits netting to exactly zero across five add-then-revert episodes. Every revert existed only because the push happened mid-experiment.)
