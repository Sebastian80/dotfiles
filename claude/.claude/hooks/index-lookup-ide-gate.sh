...
MSG="PhpStorm is not running (the index MCP port ${PORT} does not answer), so the index-lookup worker cannot answer. Ask the user with AskUserQuestion: start PhpStorm with this project, or answer without the index. If they choose to start it, run ${HOME}/bin/phpstorm-background with the project root (it starts PhpStorm without keeping keyboard focus and returns once port ${PORT} answers), then run index-lookup again; the worker itself checks that the project is open and indexed. If they decline, or no question can be asked, answer with the built-in tools (Grep, Glob, Read, rg); in PHP projects use rg -uu, since plain rg follows .gitignore and skips vendor/."
